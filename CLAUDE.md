...
- `digest/` — Activity digest accumulation and synthesis. `DigestStore` (SQLite) records tagged one-liners as agents work, with `audience` field for separate digest streams (e.g. "transactions", "changes", "ops"). `DigestSynthesizer` pulls entries by time window or audience, formats them, and uses an LLM to produce narrative digests (with structured fallback). `DigestConfig` controls the synthesis prompt per application. Middleware: `extract_from_response()` reads `digest`/`digest_audience` from response metadata, `digest_blackboard()` hooks Blackboard posts, `digest_consensus()` hooks consensus results.
